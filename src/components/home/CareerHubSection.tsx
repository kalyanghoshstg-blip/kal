...
export const CareerHubSection: React.FC<CareerHubSectionProps> = ({
  ...
  const carouselPrograms = CAREER_PROGRAMS.filter(p => p.id !== 'cv-cover-letter' && p.id !== 'fellowship-applications');
  const staticPrograms = CAREER_PROGRAMS.filter(p => p.id === 'cv-cover-letter' || p.id === 'fellowship-applications');
  ...
  const getProgramIcon = (cat: string) => {
    switch (cat) {
      ...
      case 'CV & Cover Letter': return <FileText className="w-5 h-5 text-violet-600" />;
      case 'Fellowship Applications': return <Award className="w-5 h-5 text-amber-600" />;
  ...
  return (
    <section id="career-hub" className="py-10 sm:py-14 bg-slate-50 relative border-b border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between gap-4 mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 text-[11px] font-bold">
              {carouselPrograms.length} Pathways
            </span>
          </div>
              ...
              >
                <div>
                  ...
                  <div className="space-y-1.5 text-[11px] text-slate-700 mb-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    ...
                    {prog.keyOfferings.slice(0, 4).map((off, idx) => (
        ...
        <div className="flex sm:hidden items-center justify-center gap-1 text-[11px] text-slate-400 mt-2 mb-8">
          <span>Swipe horizontally or use arrows to view all career programs</span>
        </div>

        {/* Standalone Individual Sections (CV & Fellowship) */}
        {staticPrograms.length > 0 && (
          <div className="mt-12 pt-10 border-t border-slate-200">
            <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
              <span className="w-1.5 h-6 rounded bg-indigo-600 block"></span>
              Core Career Prep & Fellowships
            </h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
              {staticPrograms.map((prog) => (
                <div 
                  key={prog.id}
                  className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 hover:border-indigo-400 hover:shadow-lg transition-all duration-300 flex flex-col justify-between group/card"
                >
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-800 border border-slate-200/60 uppercase tracking-wide">
                        {prog.category}
                      </span>
                    </div>

                    <h4 className="text-lg sm:text-xl font-extrabold text-slate-900 mb-2 leading-snug group-hover/card:text-indigo-600 transition-colors">
                      {prog.title}
                    </h4>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-6">
                      {prog.targetAudience.map((aud, idx) => (
                        <span
                          key={idx}
                          className="text-[10px] sm:text-xs px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 font-semibold border border-blue-200/60"
                        >
                          {aud}
                        </span>
                      ))}
                    ...
                    {/* Key offerings */}
                    <div className="space-y-2.5 text-xs text-slate-700 mb-6 bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-100">
                      <div className="font-extrabold text-slate-900 text-[11px] uppercase tracking-wider mb-2">
                        Key Mentorship Focus Areas:
                      </div>
                      {prog.keyOfferings.map((off, idx) => (
                        <div key={idx} className="flex items-start gap-2">
                          <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                          <span className="leading-relaxed text-slate-600 font-medium">{off}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="pt-2">
                    <button
                      onClick={() => onOpenConsultation(prog.title, 'Career Mentorship')}
                      className="w-full py-3 px-4 rounded-xl bg-slate-950 hover:bg-indigo-600 text-white font-bold text-xs sm:text-sm transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md shadow-slate-950/5 hover:shadow-indigo-500/10 active:scale-98"
                    >
                      <span>Book Special Prep Session</span>
